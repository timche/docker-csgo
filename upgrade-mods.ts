...
const serverSourceModPath = path.join(__dirname, 'sourcemod', 'server_sourcemod.sh')
...
async function upgradeMod(
  ...
  if (match) {
    ...
    let changedServerSourceModFile = serverSourceModFile.replace(versionReplaceRegExp, `$1${version}$2`)
    changedServerSourceModFile = serverSourceModFile.replace(buildNumberReplaceRegExp, `$1${buildNumber}$2`)
    ...
    if (serverSourceModFile === changedServerSourceModFile) {
      ...
      if (process.env.CI === 'true') {
        await fs.writeFile(serverSourceModPath, serverSourceModFile)
        await execa('git', ['add', '.'])
